...
def abandon_t3_run(workspace_root: Path, campaign: Campaign) -> tuple[CampaignState, RunLivenessReport]:
    ...
    * Only then is the failure recorded and the campaign moved to FAILED,
    ...
    if current != CampaignStateValue.RUNNING_T3:
        raise InvalidTransitionError(
            f"Only a campaign in {CampaignStateValue.RUNNING_T3.value} can have its run abandoned; "
            f"this one is in {current.value}."
        )
    ...
    if report.liveness in {RunLiveness.SUPERVISED, RunLiveness.UNKNOWN}:
        ...
        # the assumption that ends with T3 still writing into a workspace
        # Carmel has already declared finished.
    ...
    if report.liveness == RunLiveness.ORPHANED and active is not None and active.process_group_id is not None:
        ...
        if not kill_process_group(active.process_group_id, active.command, active.leader_starttime):
            raise RunStillLiveError(
                ...
                f"in {CampaignStateValue.RUNNING_T3.value} because its tool may still be running."
            )

    if active is not None:
        save_run_record(workspace_root, _abandoned_run_record(active, report.detail))
    clear_active_run(workspace_root)
    append_event(
        workspace_root / "decision_log.jsonl",
        {
            "event": "t3_run_abandoned",
...
def _abandoned_run_record(active: ActiveRun, detail: str) -> RunRecord:
    ...
    means — so it is recorded under its own failure code rather than
