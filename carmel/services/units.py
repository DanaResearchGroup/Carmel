...
def _scale_rules_v3() -> tuple[ConversionRule, ...]:
    return _scale_rules_v2() + (
        # Exact by definition (1 mm/s = 0.001 m/s); 26 RKD laminar-burning-velocity columns in
        # the pinned H2/syngas archives report it.
        ScaleRule(kind="scale", quantity=QuantityKind.VELOCITY, from_unit="mm/s", to_unit="m/s", scale="0.001"),
    )
...
TABLE_V3 = ConversionTable(
    ...
    base_units=_base_units_v1(),
    aliases=_aliases_v2(),
    rules=_identity_rules_v1() + _scale_rules_v3(),
)
"""``TABLE_V2`` plus the exact ``mm/s`` velocity scale, for the ReSpecTh flame-speed and
speciation records (:mod:`carmel.services.respecth_series`). Added alongside rather than edited
