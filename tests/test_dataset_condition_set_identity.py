...
def _condition_graph() -> SourceGraph:
    ...
    crop = SourceNode(
        ...
        # Concrete, with a fully-populated frame (present dpi and
        # render_settings), because this is the MAXIMAL fixture: an Absent here
        # would be illegal for a crop under I7 anyway, and a frame with Absent
        # optionals would leave `_walk_completeness` short-circuiting on fields
        # this fixture exists to visit.
...
def _maximal_bbox() -> BBox:
    """A bbox with every optional sub-field of its frame PRESENT, so
    ``_walk_completeness`` reaches them.

    Shared by the crop node's own ``crop_region`` and by ``_bbox_ref``'s
    locator below. They are the SAME rectangle here only because this is a
    minimal fixture: in general a crop's ``crop_region`` is measured against
    the PARENT's page frame, while a ``BBoxLocator`` targeting that crop
    addresses a region within the crop's own render (see
...
def _bbox_ref() -> SourceRef:
    return SourceRef(node_id="crop", locator=BBoxLocator(bbox=_maximal_bbox()))
...
#
# Regenerated 2026-08-21 because SourceNode gained `crop_region`, a FIGURE_CROP's
# addressing identity (I7/I8/I9 -- see carmel/schemas/datasets.py), projected on
# every node. Verified by diffing the recomputed payload against the prior pin
# key-by-key: no top-level key added or removed, the node id set unchanged, and
# each of the three nodes differs by EXACTLY the one new `crop_region` key -- a
# concrete bbox on 'crop', an Absent(not_applicable) marker on 'paper' and 'si'.
# An address move, and a BREAKING one for anything already stored; no production
# code emits a ConditionSetEnvelope source graph containing a crop.
#
# Regenerated again 2026-08-21 after review, superseding the entry above within the
# same branch: `identity_payload_version` 1 -> 2 (the projection schema changed for
# every node while the version key stayed at 1, so a v1 payload reached pydantic
# instead of the version gate), and `crop_region` is now emitted for a FIGURE_CROP
# only (on any other kind I7 admits exactly one value, so the key was a constant
# that re-addressed every stored envelope while distinguishing none). Diffed
# key-by-key against both prior pins: versus the entry above, version 1 -> 2 plus
# `crop_region` removed from 'paper' and 'si' with 'crop' byte-identical; versus
# the pre-branch pin at 499835c, the version bump plus exactly one key added on
# 'crop', every other node byte-identical to before this branch. See the sixteenth
# regeneration note in tests/test_dataset_identity_payload.py for the full
# reasoning -- the two pins move together because they share one node projection.
#
# Regenerated 2026-08-23 because Series gained `digitization_sha256` and
# DatasetEnvelope gained `figure_digitizations` (figure-lane citation, see
# carmel/schemas/datasets.py) -- ConditionSetEnvelope has neither field, but
# both classes share `_SUPPORTED_IDENTITY_PAYLOAD_VERSION`, which bumped 2 ->
# 3 as part of that change. Verified by diffing the recomputed payload
# against the prior pin key-by-key: the ONLY delta is
# `identity_payload_version` 2 -> 3. Nothing else moved. This is an address
# move and is BREAKING for anything already stored under the old version; no
# production code emits a ConditionSetEnvelope today, so nothing is orphaned.
# Regenerated 2026-08-23 because the EMBEDDED INVENTORY payload -- not this envelope's
# own projection -- moved to INVENTORY_PAYLOAD_VERSION 2: the sub-fragment split stores
# member records (parent fragment digest + glyph/text range) where v1 stored bare
# digests, and T1 refuses any embedded record that is not the current version, so the
# fixture inventory had to move and its content address moved with it. Verified by
# diffing the recomputed payload against the prior pin key-by-key: the top-level key
# set is unchanged, and masking exactly three things -- each `table_inventories`
# entry's `canonical_json`/`inventory_sha256` and every locator's
# `pdf_table_inventory_sha256` -- makes the two payloads COMPARE EQUAL; inside the
# embedded record only `payload_version` (1 -> 2) and `cells` (member_digests ->
# members) differ. Nothing is stored under the v1 record shape in any workspace
# (measured before the bump), so this is an address move with nothing to migrate.
# Merged 2026-08-25 (official/main <- this branch): the two regenerations above
# landed in ONE payload. The envelope's own `identity_payload_version` bumped 2 ->
# 3 (this branch, the figure-citation surface) AND the EMBEDDED inventory moved to
# payload_version 2 (PR #19, sub-fragment member records), which re-addressed the
# fixture inventory and cascaded its new `inventory_sha256` into every locator's
# `pdf_table_inventory_sha256`. Recomputed from the merged schema and diffed
# key-by-key against BOTH prior pins: versus the HEAD pin the ONLY moves are the
# inventory-shape fields (each entry's canonical_json/inventory_sha256 and every
# locator's pdf_table_inventory_sha256, plus the embedded record's payload_version
# 1 -> 2 and member shape); versus the official/main pin the ONLY move is
# identity_payload_version 2 -> 3. Both bumps are present and nothing else moved.
#
# Regenerated 2026-08-25 (I-014, R-012 §3), a THIRD independent bump onto the merge above:
# the EMBEDDED INVENTORY moved to INVENTORY_PAYLOAD_VERSION 3 (a member-specific refusal now
# records `member_digests`, structured identity of its offending fragments, so a stored
# refusal reproduces only when the same offending glyphs are present). This branch was rebased
# onto an official/main that had already taken identity_payload_version to 3 and the embedded
# record to payload_version 2, so taking either conflict side would have DROPPED one bump.
# Recomputed from the merged schema and diffed key-by-key against BOTH parents
# (scratchpad/regen_verify_twosided.py, run before this commit): versus official/main the ONLY
# moves are the embedded `payload_version` (2 -> 3) and its inventory_sha256 /
# pdf_table_inventory_sha256 cascade (six leaves); versus the pre-rebase HEAD pin the ONLY
# move is `identity_payload_version` (2 -> 3, no added keys -- ConditionSetEnvelope's
# projection is unchanged by PR#20). Both surviving bumps -- identity_payload_version 3 AND
# embedded payload_version 3 -- are asserted present, and no other key moved. Nothing stored
# under the v2 embedded shape in any workspace, so this is an address move with nothing to
# migrate.
# Regenerated 2026-08-27 (I-028): UnextractedConditionStatement gained
# `statement_raw` -- the refused statement's own verbatim words, projected beside
# its `statement_ref` -- and the shared `_SUPPORTED_IDENTITY_PAYLOAD_VERSION`
# bumped 3 -> 4 (version-history entry 4 in carmel/schemas/datasets.py; a
# DatasetEnvelope has no `unextracted` and so gains no key, but stamps 4 too by
# the same shared-projection rule entry 3 records). Verified by recomputing the
# payload and diffing key-by-key against the prior pin (probes/regen_pin.py): the
# ONLY moves are `identity_payload_version` 3 -> 4 and one ADDED key
# `unextracted[0].statement_raw` = "0.6-1.0". Nothing else moved. This is an
# address move; no production code emits a ConditionSetEnvelope today and nothing
# is stored under the v3 shape, so nothing is orphaned.
# Regenerated 2026-08-31 (I-060): MeasuredValue gained `unit_provenance` and made
# `unit_raw`/`unit_ref` Maybe, and `_SUPPORTED_IDENTITY_PAYLOAD_VERSION` bumped
# 4 -> 5 (version-history entry 5). A ConditionSetEnvelope carries MeasuredValues in
# its scalar_claims, so its projection DOES move: each scalar-claim value gains
# `"unit_provenance": "printed_in_source"` (every one is printed), and the version
# stamps 5. Unlike every prior bump, this one IS breaking for a stored artifact: a
# live workspace holds a v4 condition set (and a v4 dataset), both orphaned by the
# bump (the version gate refuses v4, fail-closed). The operator authorised this;
# neither is migrated here.
# Regenerated 2026-09-05 (I-075): `SourceNode` gained `document_kind`, projected
# CONDITIONALLY like `crop_region`, and `_SUPPORTED_IDENTITY_PAYLOAD_VERSION` bumped
# 5 -> 6 (version-history entry 6). The `si` node here declares NO document_kind, so
# the conditional key is OMITTED and NO node payload moves; the ONLY change from the
# prior pin is `identity_payload_version` 5 -> 6 (verified by recomputing and diffing:
# a single-byte move at that one token, nothing else). The two v4 artifacts entry 5
# names were already orphaned by the v5 gate and remain the only stored envelopes;
# nothing is stored at v5, so this bump orphans nothing new.
# Regenerated 2026-09-05 (I-054): the OOXML (.docx) table lane gained a stored home --
# TableCellLocator gained `ooxml_table_inventory_sha256`, ConditionSetEnvelope gained a
# top-level `ooxml_table_inventories` carrier, V8b/T4b/T5b were added, and
# `_SUPPORTED_IDENTITY_PAYLOAD_VERSION` bumped 6 -> 7. Verified mechanically
# (scratchpad/regen_goldens.py): rewinding the recomputed payload -- drop
# `ooxml_table_inventories`, drop each table_cell locator's `ooxml_table_inventory_sha256`,
# set version back to 6 -- compares BYTE-EQUAL to the prior pin. So the ONLY deltas are
# `identity_payload_version` 6 -> 7, the new empty `ooxml_table_inventories` (this fixture
# cites no .docx grid), and one Absent(not_applicable) `ooxml_table_inventory_sha256` per
# table_cell locator (sorted ahead of `pdf_table_inventory_sha256`). Nothing else moved;
# BREAKING for anything stored at v6, of which there is nothing (the only stored envelopes
# are the v4 pair, already orphaned by the v5 gate).
_PINNED_CANONICAL_BYTES = b'{"attribution":"own_experiment","attribution_ref":{"locator":{"end":20,"kind":"char_span","start":10,"text_space":"extracted_text"},"node_id":"paper"},"categorical_claims":[{"claim_id":"diluent","label_raw":"diluent","label_ref":{"locator":{"col":1,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":"193b165aab6832b15c1c780574aa6f65bbee7589c3c67eb104df2fced81fb24b","row":0,"table_key":{"kind":"caption_label","label":"Table 1"}},"node_id":"paper"},"token_raw":"CO2","token_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"}}],"conversion_tables":[{"canonical_json":"{\\"aliases\\":[{\\"normalized\\":\\"C\\",\\"quantity\\":\\"temperature\\",\\"raw\\":\\"\xc2\xb0C\\"},{\\"normalized\\":\\"C\\",\\"quantity\\":\\"temperature\\",\\"raw\\":\\"degC\\"},{\\"normalized\\":\\"C\\",\\"quantity\\":\\"temperature\\",\\"raw\\":\\"deg C\\"},{\\"normalized\\":\\"cm/s\\",\\"quantity\\":\\"velocity\\",\\"raw\\":\\"cm s^-1\\"},{\\"normalized\\":\\"cm/s\\",\\"quantity\\":\\"velocity\\",\\"raw\\":\\"cm s-1\\"},{\\"normalized\\":\\"cm/s\\",\\"quantity\\":\\"velocity\\",\\"raw\\":\\"cm/sec\\"},{\\"normalized\\":\\"m/s\\",\\"quantity\\":\\"velocity\\",\\"raw\\":\\"m s^-1\\"},{\\"normalized\\":\\"cm3\\",\\"quantity\\":\\"volume\\",\\"raw\\":\\"
ity\\":\\"mole_fraction\\",\\"scale\\":\\"0.000001\\",\\"to_unit\\":\\"1\\"},{\\"from_unit\\":\\"ppm\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"mass_fraction\\",\\"scale\\":\\"0.000001\\",\\"to_unit\\":\\"1\\"}],\\"table_id\\":\\"carmel-unit-conversions\\",\\"version\\":1}\\n","sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122"}],"envelope_type":"condition_set","identity_payload_version":7,"ooxml_table_inventories":[],"scalar_claims":[{"claim_id":"initial_pressure","label_raw":"initial pressure","label_ref":{"locator":{"end":20,"kind":"char_span","start":10,"text_space":"extracted_text"},"node_id":"paper"},"uncertainty":{"basis":"absolute","kind":"std_dev","lower":{"canonical_decimal_value":"0.1","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"pressure","raw_text":"0.1","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"atm","unit_provenance":"printed_in_source","unit_raw":"atm","unit_ref":{"locator":{"col":1,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":"193b165aab6832b15c1c780574aa6f65bbee7589c3c67eb104df2fced81fb24b","row":0,"table_key":{"kind":"caption_label","label":"Table 1"}},"node_id":"paper"},"value_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"}},"scale":"linear","upper":{"canonical_decimal_value":"0.1","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"pressure","raw_text":"0.1","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"atm","unit_provenance":"printed_in_source","unit_raw":"atm","unit_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"},"value_ref":{"locator":{"col":1,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":"193b165aab6832b15c1c780574aa6f65bbee7589c3c67eb104df2fced81fb24b","row":0,"table_key":{"kind":"caption_label","label":"Table 1"}},"node_id":"paper"}}},"value":{"canonical_decimal_value":"1.0","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"pressure","raw_text":"1.0","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"atm","unit_provenance":"printed_in_source","unit_raw":"atm","unit_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"},"value_ref":{"locator":{"col":1,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":"193b165aab6832b15c1c780574aa6f65bbee7589c3c67eb104df2fced81fb24b","row":0,"table_key":{"kind":"caption_label","label":"Table 1"}},"node_id":"paper"}}}],"source_graph":{"nodes":[{"crop_region":{"frame":{"cropbox":["0","0","612","792"],"dpi":"300","mediabox":["0","0","612","792"],"render_fingerprint":"fp-1","render_settings":"antialias=on","rotation":0,"units":"pt"},"x0":"10","x1":"30","y0":"20","y1":"40"},"extraction":{"__absent__":true,"note":null,"reason":"not_applicable"},"glyph_health":{"__absent__":true,"note":null,"reason":"not_applicable"},"kind":"figure_crop","node_id":"crop","origin":{"__absent__":true,"note":null,"reason":"not_applicable"},"parent_node_id":"paper","sha256":"cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc","verification":{"__absent__":true,"note":null,"reason":"not_applicable"}},{"extraction":{"extracted_sha256":"ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff","extracted_text_sha256":"eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee","extraction_sha256":"5291844e7bedab416755e826cbdf2b34283de1753c7ef2a0fcae20f0dc5c2529","extractor":"pdf:pypdf","extractor_code_sha256":"1111111111111111111111111111111111111111111111111111111111111111","identity_payload_version":"2","parent_raw_sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","pypdf_version":"9.9.9-synthetic"},"glyph_health":{"assessor":{"content_sha256":"af3553a8142b50bba56b6ba164778b4cd2bff6e4916ac2e93c4e1a270ba4ab5a","dependency_id":"carmel.numeric.glyph_health","input_sha256":"eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"},"health":{"has_ascii6_uncertainty_marker":false,"has_equals_ambiguity_marker":false,"has_slash_c0_minus_marker":false,"has_thorn_plus_marker":false,"suspects_dash_corruption":false}},"kind":"paper_pdf","node_id":"paper","origin":{"__absent__":true,"note":null,"reason":"not_applicable"},"parent_node_id":null,"sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","verification":{"extracted_text":"extraction_record_digest_authenticated","raw_artifact":"raw_sha256_digest_authenticated","root_sidecar":"root_sidecar_digest_authenticated"}},{"extraction":{"__absent__":true,"note":null,"reason":"not_extracted_yet"},"glyph_health":{"__absent__":true,"note":null,"reason":"not_extracted_yet"},"kind":"si_member","node_id":"si","origin":{"archive_sha256":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"},"parent_node_id":"paper","sha256":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb","verification":{"__absent__":true,"note":null,"reason":"not_extracted_yet"}}]},"subject":{"label_raw":"spherical combustion vessel","label_ref":{"locator":{"end":20,"kind":"char_span","start":10,"text_space":"extracted_text"},"node_id":"paper"},"subject_kind":"device_class"},"table_inventories":[{"canonical_json":"{\\"cells\\":[{\\"col\\":1,\\"members\\":[{\\"fragment_sha256\\":\\"ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff\\",\\"glyph_end\\":null,\\"glyph_start\\":null,\\"text\\":\\"r0c1\\",\\"x_end\\":\\"0x1.3000000000000p+4\\",\\"x_start\\":\\"0x1.4000000000000p+3\\"}],\\"row\\":0,\\"text\\":\\"r0c1\\",\\"x_end\\":\\"0x1.3000000000000p+4\\",\\"x_start\\":\\"0x1.4000000000000p+3\\"}],\\"column_bounds\\":[[\\"0x0.0p+0\\",\\"0x1.f400000000000p+8\\"]],\\"footprint\\":{\\"caption_baseline_y\\":\\"0x1.5e00000000000p+9\\",\\"caption_text\\":\\"Table 1. A fixture, not a table.\\",\\"caption_x_start\\":\\"0x1.2000000000000p+6\\",\\"geometry_origin\\":\\"hand_drawn\\",\\"page\\":0,\\"x_end\\":\\"0x1.f400000000000p+8\\",\\"x_start\\":\\"0x1.2000000000000p+6\\",\\"y_bottom\\":\\"0x1.9000000000000p+7\\",\\"y_top\\":\\"0x1.5900000000000p+9\\"},\\"fragment_geometry_sha256\\":\\"ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff\\",\\"inventory_code_sha256\\":\\"ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff\\",\\"payload_version\\":4,\\"pypdf_version\\":\\"0.0.0-fixture\\",\\"raw_sha256\\":\\"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\\",\\"refusals\\":[],\\"rows\\":[{\\"anchor_text\\":\\"row 0\\",\\"anchor_x_start\\":\\"0x1.2000000000000p+6\\",\\"baseline_y\\":\\"0x1.2c00000000000p+9\\",\\"merged_baselines\\":[],\\"ordinal\\":0}]}\\n","inventory_sha256":"193b165aab6832b15c1c780574aa6f65bbee7589c3c67eb104df2fced81fb24b","raw_sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}],"unextracted":[{"label_raw":"equivalence ratio","label_ref":{"locator":{"end":20,"kind":"char_span","start":10,"text_space":"extracted_text"},"node_id":"paper"},"quantity_kind":"equivalence_ratio","reason":"value_range","statement_id":"phi_range","statement_raw":"0.6-1.0","statement_ref":{"locator":{"bbox":{"frame":{"cropbox":["0","0","612","792"],"dpi":"300","mediabox":["0","0","612","792"],"render_fingerprint":"fp-1","render_settings":"antialias=on","rotation":0,"units":"pt"},"x0":"10","x1":"30","y0":"20","y1":"40"},"kind":"bbox"},"node_id":"crop"}}]}\n'  # noqa: E501
...
class TestFromIdentityPayloadRoundTrip:
    ...
    def test_a_declared_member_payload_round_trips_byte_for_byte(self) -> None:
        """I-075: the ConditionSetEnvelope side uses the SAME conditional
        projection and inverse as the DatasetEnvelope side, so a member that
        DECLARES its document_kind must also project the key and parse back
        byte-identically -- not just the undeclared fixture above."""
        graph = _condition_graph()
        declared_nodes = tuple(
            node.model_copy(update={"document_kind": SiMemberDocumentKind.SPREADSHEET})
            if node.kind is SourceNodeKind.SI_MEMBER
            else node
            for node in graph.nodes
        )
        env = _maximal_condition_set_envelope(source_graph=SourceGraph(nodes=declared_nodes))
        payload = env.identity_payload()
        member_payload = next(n for n in payload["source_graph"]["nodes"] if n["kind"] == "si_member")
        assert member_payload["document_kind"] == "spreadsheet"

        rebuilt = ConditionSetEnvelope.from_identity_payload(payload)

        assert canonical_json_bytes(rebuilt.identity_payload()) == canonical_json_bytes(payload)
        member = next(n for n in rebuilt.source_graph.nodes if n.kind is SourceNodeKind.SI_MEMBER)
        assert member.document_kind is SiMemberDocumentKind.SPREADSHEET
