...
def _maximal_graph() -> tuple[SourceGraph, SourceRef, SourceRef, SourceRef, SourceRef, SourceRef]:
    ...
    si = SourceNode(
        ...
        verification=_verification_for(_NO_EXTRACTION),
        crop_region=_NO_CROP_REGION,
        # DECLARED (concrete) rather than left NOT_EXTRACTED_YET, so this
        # maximal fixture exercises the conditionally-projected document_kind
        # in its PRESENT state -- the crop node does the same for crop_region.
        # A .csv member is a spreadsheet, which its MemberSheetKey ref matches.
        document_kind=SiMemberDocumentKind.SPREADSHEET,
    )
    crop = SourceNode(
        node_id="crop",
    ...
    # A curated-database record read out of a pinned archive: the second kind
    # that may carry a concrete ArchiveOrigin, addressed by XPath only.
    record = SourceNode(
        node_id="record",
        kind=SourceNodeKind.DATABASE_RECORD,
        sha256=SHA_E,
        parent_node_id=None,
        origin=ArchiveOrigin(archive_sha256=SHA_F, member_display_path="x00000070_p.xml"),
    ...
    graph = SourceGraph(nodes=(crop, jats, paper, record, si))
...
def _maximal_envelope() -> DatasetEnvelope:
    ...
    record_ref = SourceRef(node_id="record", locator=XPathLocator(xpath="/experiment/dataGroup[1]/dataPoint[1]/x1[1]"))
    t_axis3 = AxisDeclaration(
        axis_id="temperature",
        role=AxisRole.COORDINATE,
        quantity_kind=QuantityKind.TEMPERATURE,
        label_raw="temperature",
        label_ref=record_ref,
    )
    tau_axis3 = AxisDeclaration(
        axis_id="tau",
        role=AxisRole.OBSERVATION,
    ...
    point3 = DataPoint(
        ...
        coordinates=(
            Coordinate(
                axis_id="temperature",
                value=_amount("1279", QuantityKind.TEMPERATURE, "K", "K", record_ref, record_ref),
        ...
        observations=(
            Observation(
                axis_id="tau",
                value=_amount("265.1", QuantityKind.TIME, "us", "us", record_ref, record_ref),
                uncertainty=Absent(reason=AbsenceReason.UNKNOWN),
            ),
        ),
    ...
    return DatasetEnvelope(
        ...
        table_inventories=cover_for(composition, (series, series2, series3)),
...
class TestProjectionCompleteness:
    ...
    def test_conditionally_projected_registry_has_exactly_the_known_kind_conditional_field(self) -> None:
        ...
        some addresses, so the set of them must stay small and deliberate. Two
        entries today -- ``SourceNode.crop_region`` (identity-bearing for a
        FIGURE_CROP, a NOT_APPLICABLE constant elsewhere) and
        ``SourceNode.document_kind`` (identity-bearing for an SI_MEMBER that
        DECLARES its kind, a kind-determined absent constant otherwise, I-075).
        Each needs its own explicit inverse in from_identity_payload()."""
        from carmel.schemas.datasets import _CONDITIONALLY_PROJECTED_FIELDS  # type: ignore[attr-defined]

        assert set(_CONDITIONALLY_PROJECTED_FIELDS) == {
...
class TestDocumentKindIsProjectedOnlyForDeclaredMembers:
    """I-075: ``SourceNode.document_kind`` is the second conditionally-projected
    field, following ``crop_region`` exactly -- emitted only when an SI member
    DECLARES its kind, omitted when the kind-determined absence holds -- with
    one telling difference: the inverse restores the marker for EVERY node that
    omits the key (a non-member to NOT_APPLICABLE, an undeclared member to
    NOT_EXTRACTED_YET), so an SI member that omits the key is UNDECLARED, not
    malformed. The maximal fixture declares its ``si`` node SPREADSHEET, so the
    key is exercised present here and absent on the other three kinds."""

    def test_the_declared_member_projects_its_document_kind(self) -> None:
        payload = _maximal_envelope().identity_payload()
        si = next(n for n in payload["source_graph"]["nodes"] if n["kind"] == "si_member")
        assert si["document_kind"] == "spreadsheet"

    @pytest.mark.parametrize("kind", ["paper_pdf", "jats_xml", "figure_crop"])
    def test_a_node_that_declares_no_kind_does_not_project_the_key_at_all(self, kind: str) -> None:
        payload = _maximal_envelope().identity_payload()
        node = next(n for n in payload["source_graph"]["nodes"] if n["kind"] == kind)
        assert "document_kind" not in node, (
            f"a {kind} node projected document_kind -- the concept does not apply there, so its "
            "NOT_APPLICABLE marker is a constant that carries no distinguishing bits"
        )

    def test_the_fixture_exercises_both_projection_states(self) -> None:
        """Guards the two tests above from passing vacuously: the maximal
        fixture must hold at least one node that PROJECTS the key and at least
        one that OMITS it."""
        payload = _maximal_envelope().identity_payload()
        nodes = payload["source_graph"]["nodes"]
        assert any("document_kind" in n for n in nodes), "no node declares a document_kind -- present state untested"
        assert any("document_kind" not in n for n in nodes), "every node declares one -- absent state untested"

    def test_the_round_trip_is_byte_exact_and_restores_the_right_marker(self) -> None:
        """The omission is a projection SHAPE, not a lost field: the declared
        member comes back declared, and every omitting node comes back with
        exactly the one absence its kind admits -- byte-identical."""
        envelope = _maximal_envelope()
        payload = envelope.identity_payload()
        ...
        assert canonical_json_bytes(parsed.identity_payload()) == canonical_json_bytes(payload)
        for node in parsed.source_graph.nodes:
            if node.kind == SourceNodeKind.SI_MEMBER:
                assert node.document_kind is SiMemberDocumentKind.SPREADSHEET
            else:
                assert node.document_kind == Absent(reason=AbsenceReason.NOT_APPLICABLE), (
                    f"{node.kind.value} node came back with {node.document_kind!r} -- the inverse must "
                    "restore exactly the one absence _validate_document_kind admits for its kind"
                )

    def test_an_undeclared_member_omits_the_key_and_round_trips_not_refused(self) -> None:
        """The difference from crop_region: an SI member with NO document_kind
        in its bytes is UNDECLARED (restored to NOT_EXTRACTED_YET), never
        malformed -- so it round-trips byte-exact rather than being refused the
        way a crop missing its region is."""
        envelope = _minimal_envelope_with_member_display_path("si/data.csv")
        payload = envelope.identity_payload()
        si = next(n for n in payload["source_graph"]["nodes"] if n["kind"] == "si_member")
        assert "document_kind" not in si

        parsed = DatasetEnvelope.from_identity_payload(payload)

        assert canonical_json_bytes(parsed.identity_payload()) == canonical_json_bytes(payload)
        member = next(n for n in parsed.source_graph.nodes if n.kind == SourceNodeKind.SI_MEMBER)
        assert member.document_kind == Absent(reason=AbsenceReason.NOT_EXTRACTED_YET)

    def test_a_payload_declaring_a_kind_on_a_non_member_node_is_refused(self) -> None:
        """A concrete document_kind on a non-member node is refused outright by
        ``_validate_document_kind`` -- the concept does not apply there."""
        payload = copy.deepcopy(_maximal_envelope().identity_payload())
        for node in payload["source_graph"]["nodes"]:
            if node["kind"] == "paper_pdf":
                node["document_kind"] = "pdf"

        with pytest.raises(DatasetEnvelopeParseError):
            DatasetEnvelope.from_identity_payload(payload)

    def test_a_payload_carrying_the_absent_marker_on_a_non_member_node_is_refused(self) -> None:
        """The projection shape is canonical. Adding the NOT_APPLICABLE marker
        back on a PAPER_PDF validates (it is the legal absence there) but does
        not re-project to itself -- the conditional projection omits it -- so
        the round-trip byte comparison refuses the second byte-shape."""
        payload = copy.deepcopy(_maximal_envelope().identity_payload())
        for node in payload["source_graph"]["nodes"]:
            if node["kind"] == "paper_pdf":
                node["document_kind"] = {"__absent__": True, "reason": "not_applicable", "note": None}
...
def _minimal_envelope_with_member_display_path(member_display_path: str | None) -> DatasetEnvelope:
    ...
    si = SourceNode(
        ...
        verification=_verification_for(_NO_EXTRACTION),
        crop_region=_NO_CROP_REGION,
        # UNDECLARED here (the field under test is origin, not document_kind),
        # so this SI member exercises document_kind's absent/omitted state.
...
#
# SEVENTEENTH regeneration, 2026-08-23: Series gained `digitization_sha256`
# (figure-lane counterpart to TableCellLocator.pdf_table_inventory_sha256) and
# DatasetEnvelope gained `figure_digitizations` (counterpart to
# `table_inventories`), plus the associated citation validators (see
# carmel/schemas/datasets.py). `_SUPPORTED_IDENTITY_PAYLOAD_VERSION` bumped 2 ->
# 3. Verified by diffing the recomputed payload against the prior golden
# key-by-key: the ONLY deltas are the new top-level `figure_digitizations` key
# (an empty list -- this fixture's two series are TABULAR/TEXTUAL, neither
# DIGITIZED), `identity_payload_version` 2 -> 3, and a new
# `digitization_sha256` key (an Absent(not_applicable) marker) on each of the
# two series. Nothing else moved. This IS an address move and is BREAKING for
# anything already stored under the old projection; the only production
# emitter of a DatasetEnvelope today never produces a DIGITIZED series, so
# nothing this repository produces is orphaned by it.
# SEVENTEENTH regeneration, 2026-08-23, authorised by the M2 extraction brief's
# already-made identity decision: the EMBEDDED INVENTORY payload -- not this
# envelope's own projection -- moved to INVENTORY_PAYLOAD_VERSION 2 (sub-fragment
# member records: parent fragment digest + glyph/text range, replacing bare member
# digests), and T1 refuses any embedded record that is not the current version.
# Verified by diffing the recomputed payload against the prior pin key-by-key: the
# top-level key set is unchanged, and masking exactly the `table_inventories`
# entries' `canonical_json`/`inventory_sha256` plus every locator's
# `pdf_table_inventory_sha256` makes the two payloads COMPARE EQUAL; inside the
# embedded record only `payload_version` (1 -> 2) and the cells' member shape
# differ. No version-1 inventory record exists in any workspace store, so nothing
# stored is re-addressed.
# Merged 2026-08-25 (official/main <- this branch): the two SEVENTEENTH
# regenerations above landed in ONE payload. The envelope's own
# `identity_payload_version` bumped 2 -> 3 with the new top-level
# `figure_digitizations` (empty -- both fixture series are TABULAR/TEXTUAL) and a
# `digitization_sha256` Absent marker per series (this branch) AND the EMBEDDED
# inventory moved to payload_version 2 with its sha cascaded into every locator
# (PR #19). Recomputed from the merged schema and diffed key-by-key against BOTH
# prior pins: versus the HEAD golden the ONLY moves are the inventory-shape fields;
# versus the official/main golden the ONLY moves are identity_payload_version
# 2 -> 3, the new figure_digitizations key, and the per-series digitization_sha256.
# Both bumps are present and nothing else moved.
#
# EIGHTEENTH regeneration, 2026-08-25 (I-014, R-012 §3), reconciling a THIRD independent
# bump onto the merge above: the EMBEDDED INVENTORY moved to INVENTORY_PAYLOAD_VERSION 3
# (a member-specific refusal now records `member_digests`, structured identity of its
# offending fragments, so a stored refusal reproduces only when the same offending glyphs
# are present). This branch was rebased onto an official/main that had already bumped the
# envelope's own `identity_payload_version` to 3 and the embedded record to payload_version 2,
# so taking either conflict side would have DROPPED one bump. Regenerated from the merged
# schema and diffed key-by-key against BOTH parents (scratchpad/regen_verify_twosided.py, run
# before this commit): versus official/main the ONLY moves are the embedded `payload_version`
# (2 -> 3) and its inventory_sha256/pdf_table_inventory_sha256 cascade; versus the pre-rebase
# HEAD golden the ONLY moves are `identity_payload_version` (2 -> 3) plus PR#20's six
# `digitization_sha256` Absent leaves. Both surviving bumps -- identity_payload_version 3 AND
# embedded payload_version 3 -- are asserted present, and no other key moved. No inventory
# record exists in any workspace store under the v2 embedded shape, so nothing stored is
# re-addressed.
#
# NINETEENTH regeneration, 2026-08-27 (I-028): the shared
# `_SUPPORTED_IDENTITY_PAYLOAD_VERSION` bumped 3 -> 4 because
# UnextractedConditionStatement gained `statement_raw` (the refused statement's own
# verbatim words). A DatasetEnvelope carries no `unextracted`, so its projection
# gains NO key -- but the version number is a single shared projection-schema
# number, so it stamps 4 too, by the same rule the SEVENTEENTH entry records for
# the figure-lane bump. Verified by diffing the recomputed payload against the
# prior golden key-by-key: the ONLY delta is `identity_payload_version` 3 -> 4.
# Nothing else moved. This IS an address move and is BREAKING for anything stored
# under v3; no production code emits a DatasetEnvelope under any version and
# nothing is stored, so nothing is orphaned.
#
# TWENTIETH regeneration, 2026-08-31 (I-060): `_SUPPORTED_IDENTITY_PAYLOAD_VERSION`
# bumped 4 -> 5 because MeasuredValue gained `unit_provenance` and made
# `unit_raw`/`unit_ref` Maybe (a dimensionless coordinate may now declare
# EQUIVALENCE_RATIO with the unit marked not-printed-in-source). Every MeasuredValue
# in this maximal fixture is PRINTED, so the deltas versus the prior golden are
# exactly: `identity_payload_version` 4 -> 5, and one `"unit_provenance":
# "printed_in_source"` key added to each MeasuredValue (unit_raw/unit_ref values are
# unchanged for a printed value). Unlike every prior bump, this one IS breaking for
# stored artifacts: a live workspace holds a v4 dataset and a v4 condition set, and
# both are orphaned by the bump (the version gate refuses to load v4, fail-closed).
# The operator authorised this knowing it; neither is migrated here.
#
# TWENTY-FIRST regeneration, 2026-09-05 (I-075): SourceNode gained `document_kind`
# (a supplementary member may DECLARE whether it is a PDF, a word-processor document,
# or a spreadsheet), projected CONDITIONALLY like `crop_region` -- emitted only when a
# concrete kind is declared -- and `_SUPPORTED_IDENTITY_PAYLOAD_VERSION` bumped 5 -> 6.
# The maximal fixture's `si` node now DECLARES SiMemberDocumentKind.SPREADSHEET (a .csv
# member, matching its MemberSheetKey ref), so this fixture exercises the conditional
# key in its PRESENT state, exactly as the `crop` node does for `crop_region`; the
# other three nodes declare nothing applicable and project NO key. Verified by
# recomputing `canonical_json_bytes(_maximal_envelope().identity_payload())` and diffing
# against the prior golden: the ONLY deltas are (a) `identity_payload_version` 5 -> 6 and
# (b) exactly one `"document_kind":"spreadsheet"` key added to the `si` node (sorted
# first, ahead of `extraction`); every other node and every other key is byte-identical.
# The two v4 artifacts the TWENTIETH entry names were already orphaned by the v5 gate and
# remain the only stored envelopes; nothing is stored at v5, so this bump orphans nothing
# new. Not precedent: a SURPRISE failure still means an address moved and must be reverted.
#
# TWENTY-SECOND regeneration, 2026-09-05 (I-054): the OOXML (.docx) table lane gained a
# stored home. TableCellLocator gained `ooxml_table_inventory_sha256` (the word-processor
# counterpart to `pdf_table_inventory_sha256`), DatasetEnvelope and ConditionSetEnvelope
# each gained a top-level `ooxml_table_inventories` carrier, and V8b/T4b/T5b were added; the
# shared `_SUPPORTED_IDENTITY_PAYLOAD_VERSION` bumped 6 -> 7. Verified mechanically (see
# scratchpad/regen_goldens.py, run before this commit): the recomputed payload rewound by
# dropping the new `ooxml_table_inventories` key, dropping every table_cell locator's new
# `ooxml_table_inventory_sha256` key, and setting the version back to 6 compares BYTE-EQUAL
# to the prior golden. So the ONLY deltas are (a) `identity_payload_version` 6 -> 7, (b) the
# new top-level `ooxml_table_inventories` (empty -- this fixture cites no .docx grid), and
# (c) one `ooxml_table_inventory_sha256` Absent(not_applicable) marker per table_cell locator
# (sorted ahead of `pdf_table_inventory_sha256`). Nothing else moved. This IS an address move,
# BREAKING for anything stored under v6; the two v4 artifacts remain the only stored
# envelopes and were already orphaned by the v5 gate, so nothing at v6 is orphaned anew.
#
# TWENTY-THIRD regeneration, 2026-09-25 (I-108): the ReSpecTh database lane added
# SourceNodeKind.DATABASE_RECORD and SourceForm.STRUCTURED_RECORD -- additive enum members,
# no new field, NO version bump. The schema edits were run against the UNCHANGED fixture
# first and this test passed, i.e. every pre-existing shape projects byte-identically. The
# fixture then grew one `record` DATABASE_RECORD node (with an ArchiveOrigin) and one `s3`
# STRUCTURED_RECORD series so it covers every node kind. Verified mechanically: the
# recomputed payload with the `record` node and the `s3` series dropped compares BYTE-EQUAL
# to the prior golden. So the ONLY deltas are that node and that series. Not an address move
# for anything already stored.
_GOLDEN_CANONICAL_BYTES = b'{"composition":{"basis":"mole_fraction","components":[{"amount":{"canonical_decimal_value":"0.04","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"mole_fraction","raw_text":"0.04","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"1","unit_provenance":"printed_in_source","unit_raw":"-","unit_ref":{"locator":{"bbox":{"frame":{"cropbox":["0","0","612","792"],"dpi":"300","mediabox":["0","0","612","792"],"render_fingerprint":"fp-1","render_settings":"antialias=on","rotation":0,"units":"pt"},"x0":"10","x1":"30","y0":"20","y1":"40"},"kind":"bbox"},"node_id":"crop"},"value_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"}},"role":"fuel","species_raw_name":"H2"}],"equivalence_ratio":{"canonical_decimal_value":"1.0","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"equivalence_ratio","raw_text":"1.0","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"1","unit_provenance":"printed_in_source","unit_raw":"-","unit_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"},"value_ref":{"locator":{"col":1,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":"ecfdc54ced3b4cff97523f0615251a3710c02d9caf88a7a11180d6cbf71c0083","row":0,"table_key":{"kind":"caption_label","label":"Table 1"}},"node_id":"paper"}},"raw_name":"4% H2 in N2","resolution":"resolved_components"},"conversion_tables":[{"canonical_json":"{\\"aliases\\":[{\\"normalized\\":\\"C\\",\\"quantity\\":\\"temperature\\",\\"raw\\":\\"\xc2\xb0C\\"},{\\"normalized\\":\\"C\\",\\"quantity\\":\\"temperature\\",\\"raw\\":\\"degC\\"},{\\"normalized\\":\\"C\\",\\"quantity\\":\\"temperature\\",\\"raw\\":\\"deg C\\"},{\\"normalized\\":\\"cm/s\\",\\"quantity\\":\\"velocity\\",\\"raw\\":\\"cm s^-1\\"},{\\"normalized\\":\\"cm/s\\",\\"quantity\\":\\"velocity\\",\\"raw\\":\\"cm s-1\\"},{\\"normalized\\":\\"cm/s\\",\\"quantity\\":\\"velocity\\",\\"raw\\":\\"cm/sec\\"},{\\"normalized\\":\\"m/s\\",\\"quantity\\":\\"velocity\\",\\"raw\\":\\"m s^-1\\"},{\\"normalized\\":\\"cm3\\",\\"quantity\\":\\"volume\\",\\"raw\\":\\"cm^3\\"},{\\"normalized\\":\\"cm3\\",\\"quantity\\":\\"volume\\",\\"raw\\":\\"cc\\"},{\\"normalized\\":\\"L\\",\\"quantity\\":\\"volume\\",\\"raw\\":\\"l\\"},{\\"normalized\\":\\"L\\",\\"quantity\\":\\"volume\\",\\"raw\\":\\"liter\\"},{\\"normalized\\":\\"L\\",\\"quantity\\":\\"volume\\",\\"raw\\":\\"litre\\"},{\\"normalized\\":\\"us\\",\\"quantity\\":\\"time\\",\\"raw\\":\\"\xc2\xb5s\\"},{\\"normalized\\":\\"us\\",\\"quantity\\":\\"time\\",\\"raw\\":\\"\xce\xbcs\\"},{\\"normalized\\":\\"%\\",\\"quantity\\":\\"mole_fraction\\",\\"raw\\":\\"percent\\"},{\\"normalized\\":\\"1\\",\\"quantity\\":\\"mole_fraction\\",\\"raw\\":\\"-\\"},{\\"normalized\\":\\"1\\",\\"quantity\\":\\"mole_fraction\\",\\"raw\\":\\"dimensionless\\"},{\\"normalized\\":\\"ppm\\",\\"quantity\\":\\"mole_fraction\\",\\"raw\\":\\"ppmv\\"},{\\"normalized\\":\\"%\\",\\"quantity\\":\\"mass_fraction\\",\\"raw\\":\\"percent\\"},{\\"normalized\\":\\"1\\",\\"quantity\\":\\"mass_fraction\\",\\"raw\\":\\"-\\"},{\\"normalized\\":\\"1\\",\\"quantity\\":\\"mass_fraction\\",\\"raw\\":\\"dimensionless\\"},{\\"normalized\\":\\"1\\",\\"quantity\\":\\"equivalence_ratio\\",\\"raw\\":\\"-\\"},{\\"normalized\\":\\"1\\",\\"quantity\\":\\"equivalence_ratio\\",\\"raw\\":\\"dimensionless\\"},{\\"normalized\\":\\"%\\",\\"quantity\\":\\"relative_uncertainty\\",\\"raw\\":\\"percent\\"},{\\"normalized\\":\\"1\\",\\"quantity\\":\\"relative_uncertainty\\",\\"raw\\":\\"-\\"},{\\"normalized\\":\\"1\\",\\"quantity\\":\\"relative_uncertainty\\",\\"raw\\":\\"dimensionless\\"}],\\"base_units\\":[[\\"length\\",\\"m\\"],[\\"velocity\\",\\"m/s\\"],[\\"temperature\\",\\"K\\"],[\\"pressure\\",\\"Pa\\"],[\\"time\\",\\"s\\"],[\\"volume\\",\\"m3\\"],[\\"strain_rate\\",\\"1/s\\"],[\\"mole_fraction\\",\\"1\\"],[\\"mass_fraction\\",\\"1\\"],[\\"equivalence_ratio\\",\\"1\\"],[\\"relative_uncertainty\\",\\"1\\"]],\\"rules\\":[{\\"kind\\":\\"identity\\",\\"quantity\\":\\"length\\",\\"unit\\":\\"m\\"},{\\"kind\\":\\"identity\\",\\"quantity\\":\\"velocity\\",\\"unit\\":\\"m/s\\"},{\\"kind\\":\\"identity\\",\\"quantity\\":\\"temperature\\",\\"unit\\":\\"K\\"},{\\"kind\\":\\"identity\\",\\"quantity\\":\\"pressure\\",\\"unit\\":\\"Pa\\"},{\\"kind\\":\\"identity\\",\\"quantity\\":\\"time\\",\\"unit\\":\\"s\\"},{\\"kind\\":\\"identity\\",\\"quantity\\":\\"volume\\",\\"unit\\":\\"m3\\"},{\\"kind\\":\\"identity\\",\\"quantity\\":\\"strain_rate\\",\\"unit\\":\\"1/s\\"},{\\"kind\\":\\"identity\\",\\"quantity\\":\\"mole_fraction\\",\\"unit\\":\\"1\\"},{\\"kind\\":\\"identity\\",\\"quantity\\":\\"mass_fraction\\",\\"unit\\":\\"1\\"},{\\"kind\\":\\"identity\\",\\"quantity\\":\\"equivalence_ratio\\",\\"unit\\":\\"1\\"},{\\"kind\\":\\"identity\\",\\"quantity\\":\\"relative_uncertainty\\",\\"unit\\":\\"1\\"},{\\"from_unit\\":\\"cm\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"length\\",\\"scale\\":\\"0.01\\",\\"to_unit\\":\\"m\\"},{\\"from_unit\\":\\"mm\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"length\\",\\"scale\\":\\"0.001\\",\\"to_unit\\":\\"m\\"},{\\"from_unit\\":\\"cm/s\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"velocity\\",\\"scale\\":\\"0.01\\",\\"to_unit\\":\\"m/s\\"},{\\"from_unit\\":\\"C\\",\\"kind\\":\\"affine\\",\\"offset\\":\\"273.15\\",\\"quantity\\":\\"temperature\\",\\"scale\\":\\"1\\",\\"to_unit\\":\\"K\\"},{\\"from_unit\\":\\"atm\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"pressure\\",\\"scale\\":\\"101325\\",\\"to_unit\\":\\"Pa\\"},{\\"from_unit\\":\\"bar\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"pressure\\",\\"scale\\":\\"100000\\",\\"to_unit\\":\\"Pa\\"},{\\"from_unit\\":\\"kPa\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"pressure\\",\\"scale\\":\\"1000\\",\\"to_unit\\":\\"Pa\\"},{\\"from_unit\\":\\"MPa\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"pressure\\",\\"scale\\":\\"1000000\\",\\"to_unit\\":\\"Pa\\"},{\\"from_unit\\":\\"ms\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"time\\",\\"scale\\":\\"0.001\\",\\"to_unit\\":\\"s\\"},{\\"from_unit\\":\\"us\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"time\\",\\"scale\\":\\"0.000001\\",\\"to_unit\\":\\"s\\"},{\\"from_unit\\":\\"cm3\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"volume\\",\\"scale\\":\\"0.000001\\",\\"to_unit\\":\\"m3\\"},{\\"from_unit\\":\\"L\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"volume\\",\\"scale\\":\\"0.001\\",\\"to_unit\\":\\"m3\\"},{\\"from_unit\\":\\"%\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"mole_fraction\\",\\"scale\\":\\"0.01\\",\\"to_unit\\":\\"1\\"},{\\"from_unit\\":\\"%\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"mass_fraction\\",\\"scale\\":\\"0.01\\",\\"to_unit\\":\\"1\\"},{\\"from_unit\\":\\"%\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"relative_uncertainty\\",\\"scale\\":\\"0.01\\",\\"to_unit\\":\\"1\\"},{\\"from_unit\\":\\"ppm\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"mole_fraction\\",\\"scale\\":\\"0.000001\\",\\"to_unit\\":\\"1\\"},{\\"from_unit\\":\\"ppm\\",\\"kind\\":\\"scale\\",\\"quantity\\":\\"mass_fraction\\",\\"scale\\":\\"0.000001\\",\\"to_unit\\":\\"1\\"}],\\"table_id\\":\\"carmel-unit-conversions\\",\\"version\\":1}\\n","sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122"}],"envelope_type":"dataset","figure_digitizations":[],"identity_payload_version":7,"ooxml_table_inventories":[],"series":[{"axes":[{"axis_id":"phi","label_raw":"phi","label_ref":{"locator":{"col":1,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":"ecfdc54ced3b4cff97523f0615251a3710c02d9caf88a7a11180d6cbf71c0083","row":0,"table_key":{"kind":"caption_label","label":"Table 1"}},"node_id":"paper"},"quantity_kind":"equivalence_ratio","role":"coordinate"},{"axis_id":"sl","label_raw":"S_L","label_ref":{"locator":{"bbox":{"frame":{"cropbox":["0","0","612","792"],"dpi":"300","mediabox":["0","0","612","792"],"render_fingerprint":"fp-1","render_settings":"antialias=on","rotation":0,"units":"pt"},"x0":"10","x1":"30","y0":"20","y1":"40"},"kind":"bbox"},"node_id":"crop"},"quantity_kind":"velocity","role":"observation"},{"axis_id":"temperature","label_raw":"T","label_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"},"quantity_kind":"temperature","role":"constant"}],"constants":[{"axis_id":"temperature","uncertainty":{"basis":"absolute","kind":"std_dev","lower":{"canonical_decimal_value":"0.1","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"temperature","raw_text":"0.1","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"K","unit_provenance":"printed_in_source","unit_raw":"K","unit_ref":{"locator":{"col":1,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":"ecfdc54ced3b4cff97523f0615251a3710c02d9caf88a7a11180d6cbf71c0083","row":0,"table_key":{"kind":"caption_label","label":"Table 1"}},"node_id":"paper"},"value_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"}},"scale":"linear","upper":{"canonical_decimal_value":"0.1","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"temperature","raw_text":"0.1","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"K","unit_provenance":"printed_in_source","unit_raw":"K","unit_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"},"value_ref":{"locator":{"col":1,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":"ecfdc54ced3b4cff97523f0615251a3710c02d9caf88a7a11180d6cbf71c0083","row":0,"table_key":{"kind":"caption_label","label":"Table 1"}},"node_id":"paper"}}},"value":{"canonical_decimal_value":"298","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"temperature","raw_text":"298","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"K","unit_provenance":"printed_in_source","unit_raw":"K","unit_ref":{"locator":{"end":20,"kind":"char_span","start":10,"text_space":"extracted_text"},"node_id":"paper"},"value_ref":{"locator":{"col":1,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":"ecfdc54ced3b4cff97523f0615251a3710c02d9caf88a7a11180d6cbf71c0083","row":0,"table_key":{"kind":"caption_label","label":"Table 1"}},"node_id":"paper"}}}],"digitization_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"points":[{"composition":{"basis":"mole_fraction","components":[{"amount":{"canonical_decimal_value":"0.04","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"mole_fraction","raw_text":"0.04","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"1","unit_provenance":"printed_in_source","unit_raw":"-","unit_ref":{"locator":{"bbox":{"frame":{"cropbox":["0","0","612","792"],"dpi":"300","mediabox":["0","0","612","792"],"render_fingerprint":"fp-1","render_settings":"antialias=on","rotation":0,"units":"pt"},"x0":"10","x1":"30","y0":"20","y1":"40"},"kind":"bbox"},"node_id":"crop"},"value_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"}},"role":"fuel","species_raw_name":"H2"}],"equivalence_ratio":{"canonical_decimal_value":"1.0","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"equivalence_ratio","raw_text":"1.0","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"1","unit_provenance":"printed_in_source","unit_raw":"-","unit_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"},"value_ref":{"locator":{"col":1,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":"ecfdc54ced3b4cff97523f0615251a3710c02d9caf88a7a11180d6cbf71c0083","row":0,"table_key":{"kind":"caption_label","label":"Table 1"}},"node_id":"paper"}},"raw_name":"4% H2 in N2","resolution":"resolved_components"},"coordinates":[{"axis_id":"phi","uncertainty":{"basis":"absolute","kind":"std_dev","lower":{"canonical_decimal_value":"0.1","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"equivalence_ratio","raw_text":"0.1","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"1","unit_provenance":"printed_in_source","unit_raw":"-","unit_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"},"value_ref":{"locator":{"bbox":{"frame":{"cropbox":["0","0","612","792"],"dpi":"300","mediabox":["0","0","612","792"],"render_fingerprint":"fp-1","render_settings":"antialias=on","rotation":0,"units":"pt"},"x0":"10","x1":"30","y0":"20","y1":"40"},"kind":"bbox"},"node_id":"crop"}},"scale":"linear","upper":{"canonical_decimal_value":"0.1","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"equivalence_ratio","raw_text":"0.1","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"1","unit_provenance":"printed_in_source","unit_raw":"-","unit_ref":{"locator":{"bbox":{"frame":{"cropbox":["0","0","612","792"],"dpi":"300","mediabox":["0","0","612","792"],"render_fingerprint":"fp-1","render_settings":"antialias=on","rotation":0,"units":"pt"},"x0":"10","x1":"30","y0":"20","y1":"40"},"kind":"bbox"},"node_id":"crop"},"value_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"}}},"value":{"canonical_decimal_value":"1.0","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"equivalence_ratio","raw_text":"1.0","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"1","unit_provenance":"printed_in_source","unit_raw":"-","unit_ref":{"locator":{"bbox":{"frame":{"cropbox":["0","0","612","792"],"dpi":"300","mediabox":["0","0","612","792"],"render_fingerprint":"fp-1","render_settings":"antialias=on","rotation":0,"units":"pt"},"x0":"10","x1":"30","y0":"20","y1":"40"},"kind":"bbox"},"node_id":"crop"},"value_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"}}}],"observations":[{"axis_id":"sl","uncertainty":{"basis":"absolute","kind":"std_dev","lower":{"canonical_decimal_value":"0.1","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"velocity","raw_text":"0.1","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"cm/s","unit_provenance":"printed_in_source","unit_raw":"cm/s","unit_ref":{"locator":{"col":1,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":"ecfdc54ced3b4cff97523f0615251a3710c02d9caf88a7a11180d6cbf71c0083","row":0,"table_key":{"kind":"caption_label","label":"Table 1"}},"node_id":"paper"},"value_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"}},"scale":"linear","upper":{"canonical_decimal_value":"0.1","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"velocity","raw_text":"0.1","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"cm/s","unit_provenance":"printed_in_source","unit_raw":"cm/s","unit_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"},"value_ref":{"locator":{"col":1,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":"ecfdc54ced3b4cff97523f0615251a3710c02d9caf88a7a11180d6cbf71c0083","row":0,"table_key":{"kind":"caption_label","label":"Table 1"}},"node_id":"paper"}}},"value":{"canonical_decimal_value":"35.0","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"velocity","raw_text":"35.0","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"cm/s","unit_provenance":"printed_in_source","unit_raw":"cm/s","unit_ref":{"locator":{"col":2,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"row":1,"table_key":{"kind":"member_sheet","sheet_name":"Sheet1"}},"node_id":"si"},"value_ref":{"locator":{"col":1,"kind":"table_cell","ooxml_table_inventory_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"pdf_table_inventory_sha256":"ecfdc54ced3b4cff97523f0615251a3710c02d9caf88a7a11180d6cbf71c0083","row":0,"table_key":{"kind":"caption_label","label":"Table 1"}},"node_id":"paper"}}}],"point_id":"p1"}],"series_id":"s1","source_form":"tabular","value_origin":"experimental"},{"axes":[{"axis_id":"phi2","label_raw":"phi2","label_ref":{"locator":{"kind":"xpath","xpath":"//table/row[1]/cell[1]"},"node_id":"jats"},"quantity_kind":"equivalence_ratio","role":"coordinate"},{"axis_id":"sl2","label_raw":"sl2","label_ref":{"locator":{"kind":"xpath","xpath":"//table/row[1]/cell[1]"},"node_id":"jats"},"quantity_kind":"velocity","role":"observation"}],"constants":[],"digitization_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"points":[{"composition":{"__absent__":true,"note":null,"reason":"not_applicable"},"coordinates":[{"axis_id":"phi2","uncertainty":{"__absent__":true,"note":null,"reason":"not_reported_here"},"value":{"canonical_decimal_value":"2.0","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"equivalence_ratio","raw_text":"2.0","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"1","unit_provenance":"printed_in_source","unit_raw":"-","unit_ref":{"locator":{"kind":"xpath","xpath":"//table/row[1]/cell[1]"},"node_id":"jats"},"value_ref":{"locator":{"kind":"xpath","xpath":"//table/row[1]/cell[1]"},"node_id":"jats"}}}],"observations":[{"axis_id":"sl2","uncertainty":{"__absent__":true,"note":null,"reason":"not_reported_here"},"value":{"canonical_decimal_value":"10.0","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"velocity","raw_text":"10.0","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"cm/s","unit_provenance":"printed_in_source","unit_raw":"cm/s","unit_ref":{"locator":{"kind":"xpath","xpath":"//table/row[1]/cell[1]"},"node_id":"jats"},"value_ref":{"locator":{"kind":"xpath","xpath":"//table/row[1]/cell[1]"},"node_id":"jats"}}}],"point_id":"q1"}],"series_id":"s2","source_form":"textual","value_origin":"simulation"},{"axes":[{"axis_id":"tau","label_raw":"ignition delay","label_ref":{"locator":{"kind":"xpath","xpath":"/experiment/dataGroup[1]/dataPoint[1]/x1[1]"},"node_id":"record"},"quantity_kind":"time","role":"observation"},{"axis_id":"temperature","label_raw":"temperature","label_ref":{"locator":{"kind":"xpath","xpath":"/experiment/dataGroup[1]/dataPoint[1]/x1[1]"},"node_id":"record"},"quantity_kind":"temperature","role":"coordinate"}],"constants":[],"digitization_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"},"points":[{"composition":{"__absent__":true,"note":null,"reason":"same_as_dataset"},"coordinates":[{"axis_id":"temperature","uncertainty":{"__absent__":true,"note":null,"reason":"unknown"},"value":{"canonical_decimal_value":"1279","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"temperature","raw_text":"1279","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"K","unit_provenance":"printed_in_source","unit_raw":"K","unit_ref":{"locator":{"kind":"xpath","xpath":"/experiment/dataGroup[1]/dataPoint[1]/x1[1]"},"node_id":"record"},"value_ref":{"locator":{"kind":"xpath","xpath":"/experiment/dataGroup[1]/dataPoint[1]/x1[1]"},"node_id":"record"}}}],"observations":[{"axis_id":"tau","uncertainty":{"__absent__":true,"note":null,"reason":"unknown"},"value":{"canonical_decimal_value":"265.1","conversion_table_sha256":"1ac7a572c24b116e62fd360edc423a9bf333c35108d798f5336e91ad7b65a122","quantity_kind":"time","raw_text":"265.1","repair_dependency":{"content_sha256":"b29d34f644deff19a68e618340408839a138186a5a4229fed8babd4f22fedabb","dependency_id":"carmel.numeric.context_free_span_repair","input_sha256":{"__absent__":true,"note":null,"reason":"not_applicable"}},"repairs":[],"unit_normalized":"us","unit_provenance":"printed_in_source","unit_raw":"us","unit_ref":{"locator":{"kind":"xpath","xpath":"/experiment/dataGroup[1]/dataPoint[1]/x1[1]"},"node_id":"record"},"value_ref":{"locator":{"kind":"xpath","xpath":"/experiment/dataGroup[1]/dataPoint[1]/x1[1]"},"node_id":"record"}}}],"point_id":"r1"}],"series_id":"s3","source_form":"structured_record","value_origin":"experimental"}],"source_graph":{"nodes":[{"crop_region":{"frame":{"cropbox":["0","0","612","792"],"dpi":"300","mediabox":["0","0","612","792"],"render_fingerprint":"fp-1","render_settings":"antialias=on","rotation":0,"units":"pt"},"x0":"10","x1":"30","y0":"20","y1":"40"},"extraction":{"__absent__":true,"note":null,"reason":"not_applicable"},"glyph_health":{"__absent__":true,"note":null,"reason":"not_applicable"},"kind":"figure_crop","node_id":"crop","origin":{"__absent__":true,"note":null,"reason":"not_applicable"},"parent_node_id":"paper","sha256":"cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc","verification":{"__absent__":true,"note":null,"reason":"not_applicable"}},{"extraction":{"__absent__":true,"note":null,"reason":"not_extracted_yet"},"glyph_health":{"__absent__":true,"note":null,"reason":"not_extracted_yet"},"kind":"jats_xml","node_id":"jats","origin":{"__absent__":true,"note":null,"reason":"not_applicable"},"parent_node_id":null,"sha256":"dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd","verification":{"__absent__":true,"note":null,"reason":"not_extracted_yet"}},{"extraction":{"extracted_sha256":"ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff","extracted_text_sha256":"eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee","extraction_sha256":"5291844e7bedab416755e826cbdf2b34283de1753c7ef2a0fcae20f0dc5c2529","extractor":"pdf:pypdf","extractor_code_sha256":"1111111111111111111111111111111111111111111111111111111111111111","identity_payload_version":"2","parent_raw_sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","pypdf_version":"9.9.9-synthetic"},"glyph_health":{"assessor":{"content_sha256":"af3553a8142b50bba56b6ba164778b4cd2bff6e4916ac2e93c4e1a270ba4ab5a","dependency_id":"carmel.numeric.glyph_health","input_sha256":"eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"},"health":{"has_ascii6_uncertainty_marker":false,"has_equals_ambiguity_marker":false,"has_slash_c0_minus_marker":false,"has_thorn_plus_marker":false,"suspects_dash_corruption":false}},"kind":"paper_pdf","node_id":"paper","origin":{"__absent__":true,"note":null,"reason":"not_applicable"},"parent_node_id":null,"sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","verification":{"extracted_text":"extraction_record_digest_authenticated","raw_artifact":"raw_sha256_digest_authenticated","root_sidecar":"root_sidecar_digest_authenticated"}},{"extraction":{"__absent__":true,"note":null,"reason":"not_applicable"},"glyph_health":{"__absent__":true,"note":null,"reason":"not_applicable"},"kind":"database_record","node_id":"record","origin":{"archive_sha256":"ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"},"parent_node_id":null,"sha256":"eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee","verification":{"__absent__":true,"note":null,"reason":"not_applicable"}},{"document_kind":"spreadsheet","extraction":{"__absent__":true,"note":null,"reason":"not_extracted_yet"},"glyph_health":{"__absent__":true,"note":null,"reason":"not_extracted_yet"},"kind":"si_member","node_id":"si","origin":{"archive_sha256":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"},"parent_node_id":"paper","sha256":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb","verification":{"__absent__":true,"note":null,"reason":"not_extracted_yet"}}]},"table_inventories":[{"canonical_json":"{\\"cells\\":[{\\"col\\":0,\\"members\\":[{\\"fragment_sha256\\":\\"ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff\\",\\"glyph_end\\":null,\\"glyph_start\\":null,\\"text\\":\\"r0c0\\",\\"x_end\\":\\"0x1.2000000000000p+3\\",\\"x_start\\":\\"0x0.0p+0\\"}],\\"row\\":0,\\"text\\":\\"r0c0\\",\\"x_end\\":\\"0x1.2000000000000p+3\\",\\"x_start\\":\\"0x0.0p+0\\"},{\\"col\\":1,\\"members\\":[{\\"fragment_sha256\\":\\"ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff\\",\\"glyph_end\\":null,\\"glyph_start\\":null,\\"text\\":\\"r0c1\\",\\"x_end\\":\\"0x1.3000000000000p+4\\",\\"x_start\\":\\"0x1.4000000000000p+3\\"}],\\"row\\":0,\\"text\\":\\"r0c1\\",\\"x_end\\":\\"0x1.3000000000000p+4\\",\\"x_start\\":\\"0x1.4000000000000p+3\\"}],\\"column_bounds\\":[[\\"0x0.0p+0\\",\\"0x1.f400000000000p+8\\"]],\\"footprint\\":{\\"caption_baseline_y\\":\\"0x1.5e00000000000p+9\\",\\"caption_text\\":\\"Table 1. A fixture, not a table.\\",\\"caption_x_start\\":\\"0x1.2000000000000p+6\\",\\"geometry_origin\\":\\"hand_drawn\\",\\"page\\":0,\\"x_end\\":\\"0x1.f400000000000p+8\\",\\"x_start\\":\\"0x1.2000000000000p+6\\",\\"y_bottom\\":\\"0x1.9000000000000p+7\\",\\"y_top\\":\\"0x1.5900000000000p+9\\"},\\"fragment_geometry_sha256\\":\\"ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff\\",\\"inventory_code_sha256\\":\\"ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff\\",\\"payload_version\\":4,\\"pypdf_version\\":\\"0.0.0-fixture\\",\\"raw_sha256\\":\\"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\\",\\"refusals\\":[],\\"rows\\":[{\\"anchor_text\\":\\"row 0\\",\\"anchor_x_start\\":\\"0x1.2000000000000p+6\\",\\"baseline_y\\":\\"0x1.2c00000000000p+9\\",\\"merged_baselines\\":[],\\"ordinal\\":0}]}\\n","inventory_sha256":"ecfdc54ced3b4cff97523f0615251a3710c02d9caf88a7a11180d6cbf71c0083","raw_sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}]}\n'  # noqa: E501
