...
``derivation_binding`` (SHA_G) are deliberately four DISTINCT constants from
...
equal the same constant. ``derivation_binding`` carries a real (present)
digest here rather than `Absent`, since this is the maximal fixture and
should exercise the present branch of every projected field."""
...
def _minimal_envelope_with_glyph_health(health: GlyphHealth) -> DatasetEnvelope:
    ...
    # bug undetectably. derivation_binding is Absent here (this fixture is
    # not exercising that field, only GlyphHealth's booleans).
...
def _minimal_envelope_with_derivation_binding(derivation_binding: str | Absent) -> DatasetEnvelope:
    """Like `_minimal_envelope_with_glyph_health`, but parameterized by
    `ExtractionBinding.derivation_binding` instead of `GlyphHealth` -- used to
    prove that field is genuinely identity-bearing (projected into
    `_extraction_binding_identity_payload`, not decorative), and that it
    round-trips correctly when Absent (the legacy-artifact case: an artifact
    stored before `derivation_binding` existed carries none)."""
    extraction = ExtractionBinding(
        parent_raw_sha256=SHA_A,
        extraction_sha256=SHA_H,
        extracted_sha256=SHA_F,
...
class TestDerivationBindingIsIdentityBearing:
    """`ExtractionBinding.derivation_binding` (Defect 2) mirrors
    `StoredArtifact.derivation_binding` in carmel/schemas/literature.py --
    proof only of INTERNAL CONSISTENCY (extractor_version, raw sha256, and
    extracted sha256 all line up), never of re-derivation and never a
    defence against a forger who controls all three inputs. These tests
    prove it is genuinely projected into the content address (not
    decorative), and that an explicit `Absent` value for it (the
    legacy-artifact case: an artifact stored before this field existed)
    round-trips and validates. The field has no default -- callers must state
    the absence explicitly, so "the producer forgot" stays distinguishable
    from "genuinely never had one"."""

    def test_changing_derivation_binding_changes_the_content_address(self) -> None:
        baseline_env = _minimal_envelope_with_derivation_binding(Absent(reason=AbsenceReason.UNKNOWN))
        baseline_sha = compute_dataset_sha(baseline_env.identity_payload())

        changed_env = _minimal_envelope_with_derivation_binding(SHA_D)
        changed_sha = compute_dataset_sha(changed_env.identity_payload())

        assert changed_sha != baseline_sha, (
            "changing ExtractionBinding.derivation_binding did not change the content address -- "
            "that field is decorative, not identity-bearing"
        )

        other_env = _minimal_envelope_with_derivation_binding(SHA_B)
        other_sha = compute_dataset_sha(other_env.identity_payload())
        assert other_sha != changed_sha, (
            "two different derivation_binding values produced the same content address"
        )

    def test_absent_derivation_binding_round_trips_and_validates(self) -> None:
        """The legacy-artifact case: an artifact stored before
        `derivation_binding` existed has none recorded. `Absent` must be a
        legal value (not merely tolerated by a default, but constructible
        and re-validatable explicitly), and must project into the identity
        payload the same way every other Maybe[...] field does."""
        env = _minimal_envelope_with_derivation_binding(Absent(reason=AbsenceReason.UNKNOWN))
        extraction = env.source_graph.nodes[0].extraction
        assert not isinstance(extraction, Absent)
        assert isinstance(extraction.derivation_binding, Absent)
        assert extraction.derivation_binding.reason is AbsenceReason.UNKNOWN

        payload = env.identity_payload()
        paper_payload = next(
            node for node in payload["source_graph"]["nodes"] if node["node_id"] == "paper"  # type: ignore[index]
        )
        assert paper_payload["extraction"]["derivation_binding"] == {
            "__absent__": True,
            "note": None,
            "reason": "unknown",
        }
