...
def _maximal_envelope() -> DatasetEnvelope:
    ...
    return DatasetEnvelope(source_graph=graph, composition=composition, series=(series, series2))
...
def _minimal_envelope_with_composition(composition: Composition | Absent) -> DatasetEnvelope:
    ...
    return DatasetEnvelope(source_graph=graph, composition=composition, series=(series,))
...
class TestProjectionCompleteness:
    ...
    def test_unaddressed_fields_registry_starts_empty(self) -> None:
        """The spec is explicit: start `_UNADDRESSED_FIELDS` EMPTY if every
        field is projected -- do not invent entries. This is not a
        contradiction with the walker tests above (which populate it
        temporarily, on a throwaway scratch model, then restore it) --
        those never touch the real dict's real content."""
        from carmel.schemas.datasets import _UNADDRESSED_FIELDS  # type: ignore[attr-defined]

        assert _UNADDRESSED_FIELDS == {}, (
            "the spec requires this registry to start empty (every real field is projected); "
            f"got {_UNADDRESSED_FIELDS!r} -- if a field is genuinely unprojectable, this test's "
            "assumption is the thing to revisit, not the registry"
        )
